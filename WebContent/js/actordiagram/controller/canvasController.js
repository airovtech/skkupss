...
// Description : Actor Diagram의 Canvas의 Model과 View를 관리하는 기능.
//
try{
...
ActorDiagram.Controller.Canvas = function(mode, target, data){
	...
	this.newSelections = function(selections){
		...
		if(this.selectedObjects.length==1){
			...
			var objectProperties = this.target.parents('.js_actor_space').find('.js_object_properties').attr('canvasId', this.id).attr('objectId', model.id);
