...
SmartWorks.FormRuntime.NumberInputBuilder.build = function(config) {
	...
	if(readOnly){
		...
		$number.attr('title', $entity.attr('toolTip'));
	}
	...
	if (readOnly) {
		var $numberHiddenInput = options.container.find('#numberHiddenInput'+id);
...
//		if(!isEmpty(numberInput.parents('td.js_type_dataGrid'))) continue;
...
SmartWorks.FormRuntime.NumberInputBuilder.validate = function(numberInputs){
	...
	smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[number_input script]', null, error);
}