...
SmartWorks.FormRuntime.PercentInputBuilder.build = function(config) {
	...
	if (readOnly) {
		...
		$percent.attr('title', $entity.attr('toolTip'));
	}
	...
	if (readOnly) {
		var $percentHiddenInput = options.container.find('#percentHiddenInput'+id);
		...
		if(options.mode == 'edit'){
			if(isReadOnly == 'true'){
				if(value===''){
					...
					formValue.find('span').formatCurrency({ symbol: '' ,colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: -1, eventOnDecimalsEntered: true });
				...
				formValue.html('<input type="text"' + required + '/>');
				if(value!==''){
					formValue.find('input').attr('value', value).formatCurrency({ symbol: '' ,colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: -1, eventOnDecimalsEntered: true });
					formValue.find('input').attr('value', formValue.find('input').attr('value') + '%');
				}
				$('#percentHiddenInput'+id).remove();
...
//		if(!isEmpty(percentInput.parents('td.js_type_dataGrid'))) continue;
...
SmartWorks.FormRuntime.PercentInputBuilder.validate = function(percentInputs){
	...
	smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[percent_input script]', null, error);
}