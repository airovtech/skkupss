...
SmartWorks.FormRuntime.DateChooserBuilder.build = function(config) {
	...
	if(readOnly){
		...
		$text = $('<div class="form_value form_value_max_width" style="width:' + valueWidth + '%"><div class="icon_fb_space form_date_input"><input readonly="readonly" type="text" name="' + id + '"' + required + '><a href=""  tabindex="-1" class="js_todaypicker_button"><span class="icon_fb_date"></span></a></div></div>');
	...
	if (readOnly){
		var $dateChooserHiddenInput = options.container.find('#dateChooserHiddenInput'+id);
	...
	if(options.refreshData && (isHidden || isRequired || isReadOnly)){
		...
		if(options.mode == 'edit'){
			if(isReadOnly == 'true'){
				...
				formValue.html('<div class="icon_fb_space form_date_input"><input readonly="readonly" type="text" name="' + id + '"' + required + '><a href="" tabindex="-1" class="js_todaypicker_button"><span class="icon_fb_date"></span></a></div>');
				formValue.find('input').attr('value', value);	
				$('#dateChooserHiddenInput'+id).remove();
...
SmartWorks.FormRuntime.DateChooserBuilder.dataField = function(config){
	...
	smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[date_chooser script]', null, error);
}