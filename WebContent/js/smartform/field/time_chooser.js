...
SmartWorks.FormRuntime.TimeChooserBuilder.build = function(config) {
	...
	var $text = null;
	if(readOnly){
		$text = $('<div class="form_value form_value_max_width" style="width:' + valueWidth + '%"><span>' + (isEmpty(value) ? '&nbsp;' : value) + '</span></div>');
	}else{	
		$text = $('<div class="form_value form_value_max_width" style="width:' + valueWidth + '%"><div class="icon_fb_space form_time_input"><input readonly="readonly" type="text" name="' + id + '"' + required + '><a href="" tabindex="-1" class="js_timepicker_button"><span class="icon_fb_time"></span></a></div></div>');
	...
	if(options.refreshData && (isHidden || isRequired || isReadOnly)){
		...
		if(options.mode == 'edit'){
			if(isReadOnly == 'true'){
				...
				formValue.html('<div class="icon_fb_space form_time_input"><input readonly="readonly" type="text" name="' + id + '"' + required + '><a href="" tabindex="-1" class="js_timepicker_button"><span class="icon_fb_time"></span></a></div>');
				formValue.find('input').attr('value', value);	
				$('#timeChooserHiddenInput'+id).remove();
...
SmartWorks.FormRuntime.TimeChooserBuilder.dataField = function(config){
	...
	smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[time_chooser script]', null, error);
}