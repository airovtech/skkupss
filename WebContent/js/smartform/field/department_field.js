	...
	if(multiUsers === 'true'){
		if(!isEmpty(departments) && isEmpty(departmentsHtml)){
			for(var i=0; i<departments.length; i++)
				departmentsHtml = departmentsHtml +  "<span class='js_community_item user_select' comId='" + departments[i].comId + "' comName='" + departments[i].name + "' title='" + departments[i].name + "'>" + getDepartNameFromFullpath(departments[i].name) + "<a class='js_remove_community' href=''  tabindex='-1'>&nbsp;x</a></span>";		
		}
		href = "community_name.sw";
//		icoClass = ' class="icon_fb_departs"';
	}else if (!isEmpty(departments) && isEmpty(departmentsHtml)) {
		departmentsHtml = "<span class='js_community_item user_select' comId='" + departments[0].comId + "' comName='" + departments[0].name + "' title='" + departments[0].name + "'>" + getDepartNameFromFullpath(departments[0].name) + "<a class='js_remove_community' href=''  tabindex='-1'>&nbsp;x</a></span>";
	}
	...
	var $html = $('<div class="form_value" style="width:' + valueWidth + '%"> <div class="icon_fb_space">\
					...
					<div class="js_community_list srch_list_nowid" style="display: none"></div><span class="js_community_popup"></span><a href=""' + departPicker + ' tabindex="-1"><span ' + icoClass + '></span></a></div></div>');
		...
		if(isEmpty(departments) && !isEmpty(departmentsHtml)){
			...
			for(var i=0; i<departments.length; i++) {
				...
				if(i == departments.length - 1)
					separator = '';
				viewDepartmentsHtml = viewDepartmentsHtml + '<a href="' + href + '" title="' + departments[i].name +  '"  tabindex="-1" class="linkline"><span>' + getDepartNameFromFullpath(departments[i].name) + desc + separator + '</span></a>';
			}
		}
		$department.find('span').html(viewDepartmentsHtml);
	}else{	
		$department = $html;
		$department.attr('title', $entity.attr('toolTip'));
	}
	...
	if(!options.refreshData){
		...
		if(readOnly){
			...
			options.container.find('.form_value .js_community_names').prepend($department.find('.js_community_names .js_community_item'));
		}
	}
	...
	if(options.refreshData && (isHidden || isRequired || isReadOnly)){
		...
		if(options.mode == 'edit'){
			if(isReadOnly == 'true'){
				...
				formValue.find('span').html(viewDepartmentsHtml);
				var $departmentHiddenDiv = options.container.find('#departmentHiddenDiv' + id);
				if ($departmentHiddenDiv.length === 0) {
					options.container.append($('<div id="departmentHiddenDiv' + id + '" style="display:none"></div>').html(departmentsHtml));
				} else {
				...
				formValue.html('<div class="icon_fb_space">\
						...
						<div class="js_community_list srch_list_nowid" style="display: none"></div><span class="js_community_popup"></span><a href=""' + departPicker + '  tabindex="-1"><span ' + icoClass + '></span></a></div>');
				$('#departmentHiddenDiv'+id).remove();
...
//		if(!isEmpty(departmentField.parents('td.js_type_dataGrid'))) continue;
...
SmartWorks.FormRuntime.DepartmentFieldBuilder.dataField = function(config){
	...
	smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[department_field script]', null, error);
}