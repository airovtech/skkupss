...
function createMemoDataFields(config){
	...
	var title = options.contents;
	if(!isEmpty(options.contents))
		options.contents = options.contents.replace(/\n/g,"<br>");
...
function createForumDataFields(config){
	var options = {
			formXml : null,
			groupId : null,
			fileName : null,
...
function createReservationDataFields(config){
	var options = {
			formXml : null,
			name : null,
			startDate : null,
	...
	smartPop.showInfo(smartPop.ERROR, smartMessage.get('technicalProblemOccured') + '[sw-form_dataFields script]', null, error);
}