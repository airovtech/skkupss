...
$(function() {
	...
	$('input.js_auto_complete').live('keyup', function(e) {
		try{
			...
			if(keyCode == 8){
				if(input.attr('value') === ''){
					var lastCommunityItem = input.parents('.form_value').find('.js_community_names .js_community_item:last');
					if(lastCommunityItem.hasClass('selected')){
						lastCommunityItem.find('.js_remove_community').click();
					}else{
						lastCommunityItem.addClass('selected');
					}
					return;
				}
			}else{
				input.parents('.form_value').find('.js_community_names .js_community_item').removeClass('selected');
			}
			...
			if(keyCode==13 || keyCode==32 || keyCode==186 || keyCode==188){
				...
				var data = '<ul><li> <a href=""  tabindex="-1" comName="' + emailAddress + '" comId="' + emailAddress + '" class="js_select_community">' +
								...
								try{
									if(isEmpty($(data).find('a')) && url === 'email_address.sw' && !isEmpty(requestedValue)){
										...
										if(isEmailAddress(emailAddress)){
											data = '<ul><li> <a href=""  tabindex="-1" comName="' + emailAddress + '" comId="' + emailAddress + '" class="js_select_community">' +
													'<span>' + emailAddress + '</span></a></li></ul>';
										}
									}
	...
	$('.js_select_community').live( 'click', function(e) {
		try{
			...
			if(isEmpty(communityItems) && !isEmpty(selectedApproverInfo)){
				...
				var nextApproverInfo = $(selectedApproverInfo[index+1]);
				...
				if(!isEmpty(nextApproverInfo))
					nextApproverInfo.next().find('input').focus();
			}else{
				var isSameId = false;
