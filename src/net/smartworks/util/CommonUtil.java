...
//		InputStream stdoutStream = new BufferedInputStream(p.getInputStream());
	...
	public static File getFileRepository(String fileDirectory, String companyId, String fileDivision, Date date) {
		...
		try {
			...
			if(fileDivision.equals("Mails")){
				// ?????¬ì?¬ì?©ì????? ??´ë????¼ì??ë²???´ë????¼ë?? ?????? ?????????ë¦¬ë?? ??¤ì????????.
				storageDir = storageDir + File.separator + "cdiUser@skku.ac.kr"; 
				storage = new File(storageDir);
	...
	public static void removePreviousImage(String previousFileName, String companyId, String fileDivision) {
		try {
			...
			String thumbFilePath = imageFile.getAbsolutePath() + File.separator + previousFileName + IMAGE_TYPE_THUMB + "." + extension;
			String originalFilePath = imageFile.getAbsolutePath() + File.separator + previousFileName + IMAGE_TYPE_ORIGINAL + "." + extension;
